...
use Laravel\Nova\Fields\Repeater\Presets\JSON as BaseJSON;
...
class JSON extends BaseJSON
{
    ...
    public function set(
        ...
        $model,
        string $attribute,
        BaseRepeatableCollection $repeatables,
        ...
        $callbacks = collect($repeaterItemsInput)
            ->map(function ($item, $itemIndex) use ($request, $requestAttribute, $repeatables, $model, $attribute, $existingItems, $uniqueField) {
                ...
                $key = method_exists($repeatable, 'getKey') ? $repeatable->getKey() : $repeatable->key();

                $model->setAttribute("{$attribute}->{$itemIndex}->type", $key);
     ...
     * @param  \Illuminate\Database\Eloquent\Model|\Laravel\Nova\Support\Fluent  $model
     */
    public function get(NovaRequest $request, $model, string $attribute, BaseRepeatableCollection $repeatables): Collection
    {
        $repeatables = RepeatableCollection::make($repeatables);
